...
# -*- coding: utf-8 -*-
"""
boenet_training_matrix.py (v2.1.0 - True BFS Language Model)
...
RE_EPOCH_NUM = re.compile(r"\[epoch\s*(\d+)/(\d+)\]")
        ...
        for line in text.split('\n'):
            ...
            if m:
                try:
                    ...
                    print(f"[recorder] Parsed __SUMMARY__ JSON for run {self.run_id}")
                    return
                except json.JSONDecodeError as e:
                    print(f"[recorder][warn] JSON parse error: {e}")
                    continue
            if line.startswith("__SUMMARY__"):
                json_part = line[len("__SUMMARY__"):].strip()
                try:
                    self.trainer_summary = json.loads(json_part)
                    print(f"[recorder] Parsed __SUMMARY__ (fallback) for run {self.run_id}")
                    return
...
def run_training(train_script: str, python_exe: str, run_dir: Path, 
    ...
    if recorder.trainer_summary:
        print(f"[train] Captured __SUMMARY__ with keys: {list(recorder.trainer_summary.keys())}")
    print(f"[train] Parsed {len(recorder.metrics_by_epoch)} epochs, total={recorder.epochs_total}")
        ...
        for line in stdout_text.split('\n'):
            ...
            if m:
                try:
                    ...
                    pass
        if val_ppl is None:
            print("[infer][warn] Could not parse inference output.")
...
def build_arg_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(description="BoeNet v2.1.0 Training Matrix")
...
def main():
    ...
    print("=" * 79)
    print("BoeNet Training Matrix v2.1.0")
    print("v2.1.0 FIX: Properly extracts metrics from __SUMMARY__ JSON")
    print("=" * 79 + "\n")
    ...
    print(f"[matrix] Dataset: {dataset}, Cells: {len(matrix)}, Repeats: {repeats}, Total: {total_runs}")
    print(f"[matrix] K=0: {k0_count}, K>0: {kpos_count}")
    ...
    progress_iter = tqdm(all_runs, desc="[matrix]", unit="run", ncols=120) if _HAS_TQDM else all_runs
    ...
    try:
        for base_cfg, rep in progress_iter:
            ...
            if _HAS_TQDM:
                progress_iter.set_postfix_str(f"lam={run_cfg.get('lambda_efficiency')}")
            ...
            print(f"[matrix] run {run_counter:03d} | epochs={row.get('epochs')} | "
                  f"best_ppl={row.get('val_ppl_best')} | infer_ppl={row.get('infer_val_ppl')}")
            run_counter += 1
    finally:
        csv_f.close()
        jsonl_f.close()
    ...
    print("\n" + "=" * 80)
    print(f"[matrix] COMPLETE: {run_counter} runs, {_format_duration(time.time() - start_time)}")
